...
void surface_put_pixel(Surface* s, int x, int y, uint32 color)
{
    ...
    if (gfx_put_pixel(GFX_CANVAS(s->pixels, s->width, s->height), x, y, color))
        s->dirty = true;
}
...
void surface_clear(Surface* s, uint32 color)
{
    ...
    gfx_fill(GFX_CANVAS(s->pixels, s->width, s->height), color);
    s->dirty = true;
}
...
void surface_fill_rect(Surface* s, int x, int y, int w, int h, uint32 color)
{
    if (!s || !s->pixels)
        return;
    gfx_fill_rect(GFX_CANVAS(s->pixels, s->width, s->height), x, y, w, h, color);
    s->dirty = true;
}
